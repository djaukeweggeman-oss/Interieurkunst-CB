...
export function CommissionForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/commission-requests", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
      ...
      form.reset();
      setState({ ok: true, message: result.demo ? "Je aanvraag is lokaal getest. Koppel Supabase om aanvragen werkelijk op te slaan." : "Dank je. Carolien neemt zo snel mogelijk contact met je op." });
    } catch (error) {
  ...
  return (
    <form className="editorial-form" onSubmit={submit} onFocusCapture={() => setStartedAt((current) => current || Date.now())}>
      ...
      <div className="form-grid"><label>Telefoonnummer<input name="phone" type="tel" autoComplete="tel" /></label><label>Soort opdracht *<select name="type" required defaultValue=""><option value="" disabled>Maak een keuze</option><option>Portret</option><option>Huisdier</option><option>Abstract werk</option><option>Anders</option></select></label></div>
      <div className="form-grid"><label>Gewenst formaat<input name="size" placeholder="Bijvoorbeeld 80 × 100 cm" /></label><label>Gewenste datum<input name="desiredDate" type="date" /></label></div>
      <label>Stijl of kleuren<textarea name="style" rows={3} placeholder="Welke sfeer, stijl of kleuren passen bij jou?" /></label>
      ...
      <p className="form-privacy">Een referentiefoto kan na het eerste contact veilig worden toegevoegd. Verstuur hier nog geen gevoelige informatie.</p>
